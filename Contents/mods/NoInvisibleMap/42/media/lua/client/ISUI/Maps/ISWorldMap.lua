...
--**                    THE INDIE STONE                    **
--***********************************************************
-- Handmade maps override
-- Changes were made so players are hidden and no longer reveal the fog of war
-- Changes were made at
--[[

	278: block commented out,
	327: commented out,
	...
	982: new method to add all map data
...
function ISWorldMap:onCenterOnPlayer()
	...
	local firstMap = mapData.mapRegions[1]
	
	local focusX = math.floor((firstMap.minX + firstMap.maxX) / 2)
	local focusY = math.floor((firstMap.minY + firstMap.maxY) / 2)
...
function ISWorldMap:setShowCellGrid(show)
	self.showCellGrid = false --override
	self.mapAPI:setBoolean("CellGrid", false)
end

function ISWorldMap:setShowTileGrid(show)
	self.showTileGrid = false --override
	self.mapAPI:setBoolean("TileGrid", false)
end
...
function ISWorldMap:close()
	ISWorldMap_instance:NIM_UpdateData()
...
function ISWorldMap.NIM_AddData()
...
function ISWorldMap.NIM_UpdateData()
	local symbolsAPI = ISWorldMap_instance.mapAPI:getSymbolsAPI()
	local playerModData = getPlayer():getModData()
	local playerInventory = getPlayer():getInventory()
...
function ISWorldMap.ShowWorldMap(playerNum, centerX, centerY, zoom)
	...
	if not ISWorldMap_instance then
		...
		ISWorldMap_instance:NIM_AddData()
...
function ISWorldMap.onKeyReleased(key)
	...
	if not playerInv:contains("HandmadeMap") then return end
